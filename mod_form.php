...
class mod_mumie_mod_form extends moodleform_mod {
    ...
    private function add_property_selection(): void {
        ...
        $taskproperties = [
            ...
            [get_string('mumie_duration_selector', 'mod_mumie'), "duration_selector"],
        ];
        ...
        foreach ($taskproperties as $taskproperty) {
            ...
            $row = new html_table_row([$label, $checkboxhtml]);
            if ($value === 'duration_selector') {
                $row->attributes['id'] = 'mumie_duration_property_row';
            }
            $table->data[] = $row;
        }
